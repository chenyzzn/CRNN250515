import sys
from pyk4a import PyK4A, Config, DepthMode, ImageFormat, transformation
import cv2
import numpy as np
import time
from ultralytics import YOLO
import threading

from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QVBoxLayout, QPushButton
from PyQt5.QtGui import QImage, QPixmap
from PyQt5.QtCore import QTimer, Qt

model = YOLO("C:\\car\\AI影像辨識\\yolo\\runs\\detect\\train\\weights\\best.pt")

def yolo(img0:np.ndarray) -> int:
    '''切割效期字樣區塊'''          # 最多可以偵測到物件數量 #信心 #如果兩個偵測框的重疊區域大於10%，它們可能會被合併或視為同一物件 #邊框寬度
    results1 = model.predict(img0, max_det = 1,conf = 0.3, #results1包含了可能的偵測結果(多個Result 物件)，每個結果內多個偵測框，每個偵測框中包含了物體的座標，信心等
                             iou = 0.1 , line_width=1) #對圖像進行物件偵測後的所有信息封裝在 Results 物件類型
    boxs = results1[0].boxes.xyxy #取出預測結果的第一個 Result 物件的多個(x1, y1, x2, y2)
    for result in results1:
        for c in result.boxes.cls: # class
            cl = str(int(c))
    #yolo_conf=result.boxes.conf[0]

    if(len(boxs) == 0): #box 為空
        return
    
    box=boxs[0] 
    x1 = int(box[0]) #(box[0]-8)
    y1 = int(box[1])
    x2 = int(box[2])
    y2 = int(box[3])

    return x1, x2, y1, y2
# ----------------------------------

def drawBox(posResult, cap_array, t_depth_image, xh, yh, zh):
    x1, x2, y1, y2 = posResult
    #print(f"x1: {x1}, x2: {x2}, y1: {y1}, y2: {y2}")
    xc = int((x1+x2)/2)
    yc = y1+20
    # 設定矩形框顏色（綠色）和邊框寬度（2）
    color = (0, 255, 0)  # BGR 順序
    color2 = (255, 0, 0)
    thickness = 4
    # 在圖片上畫矩形框
    cv2.rectangle(cap_array, (x1, y1), (x2, y2), color, thickness)
    cv2.rectangle(cap_array, (xc-1, yc-1), (xc+1, yc+1), color2, thickness-4)
    print(f"box depth: {t_depth_image[y1+50][xc]} mm")
# ---------------------------------------------------------------------------------------------

class ImageLabel(QLabel): # QLabel的子類別
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setMouseTracking(True)  #(True)不需要按著就能觸發mouseMoveEvent

    def mouseMoveEvent(self, event):
        # 滑鼠在 QLabel 上的座標
        x = event.pos().x()
        y = event.pos().y()
        self.parent().mouse_pos = (x, y)  # 把滑鼠位置傳給 CameraWindow

    def mousePressEvent(self, event):
        x = event.pos().x()
        y = event.pos().y()
        self.parent().mouse_pos = (x, y)
        print(f"滑鼠位置：{x},{y}")

        if event.button() == Qt.LeftButton:
            depth = self.parent().latest_depth[y][x]

            xh = (-0.8158) * x + 1196.74
            yh = (-1.0172) * depth + 1315.3
            zh = (-0.1587) * y + 96.893
            print(f"滑鼠座標：{xh},{yh},{zh}")

            self.parent().xh = min(max(xh, 400), 650)
            self.parent().yh = min(max(yh, 100), 550)
            self.parent().zh = min(max(zh, 40), 200)

            result = yolo(self.parent().latest_frame)
            if result is not None:
                x1, x2, y1, y2 = result
                width = round((x2 - x1) / 11.4, 3)
                height = round((411.5 - (y1 + y2)/2) / 11.932, 3)
            else:
                width = 0.0
                height = 0.0

            try:
                with open('triangleTop.txt', 'w') as f:
                    f.write(f"{self.parent().xh}\n")  # x
                    f.write(f"{self.parent().yh}\n")  # y
                    f.write(f"{self.parent().zh}\n")  # z
                    f.write("180\n")                  # rx
                    f.write("0\n")                    # ry
                    f.write("0\n")                    # rz
                    f.write(f"{width}\n")             # width
                    f.write(f"{height}\n")            # height
                print("已更新")
            except Exception as e:
                print(f"寫入失敗：{e}")
#------------------------------------------------------------------------------------------------

class CameraWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OpenCV 相機畫面嵌入 PyQt5")
        self.xh = self.yh = self.zh = 0
        self.click_valid = False

        # 建立 QLabel 來顯示影像
        self.label = ImageLabel(self) # CameraWindow的子元件
        layout = QVBoxLayout()
        self.text_label = QLabel("---------------------------------QAQ---------------------------------")
        self.button = QPushButton("一個按鈕")
        self.button.clicked.connect(self.on_button_click)  # 綁定事件

        layout.addWidget(self.label)
        layout.addWidget(self.text_label)
        layout.addWidget(self.button)
        self.setLayout(layout)
        # ----------------------------------------------------------------------------------------------
        self.mouse_pos = None

        # 初始畫面
        self.latest_frame = np.zeros((720, 1280, 3), dtype=np.uint8)  # 預設畫面黑的
        self.latest_depth = np.zeros((720, 1280), dtype=np.uint16)
        
        # 設定 QTimer 每 10ms 更新畫面
        self.timer = QTimer()
        self.timer.timeout.connect(self.update_frame)
        self.timer.start(30) # ms

        # 啟動攝影機背景執行緒
        threading.Thread(target=self.capture_loop, daemon=True).start()
        
        
    def on_button_click(self):
        self.text_label.setText("----------------------- pushed ----------------------- ( ͡° ͜ʖ ͡°)")                    

    def capture_loop(self):  #Microsoft深度攝影機    
        try:
            # 開啟 Kinect
            self.k4a = PyK4A() # 建立PyK4A物件
            self.k4a.start()
        except Exception as e:
            print("Kinect 無法啟動")
            print(e)

        while True:
            time.sleep(0.1)
            start=time.time()   
            
            capture = self.k4a.get_capture() # 讀取影片的每一幀
            color_image = capture.color # 取得彩色影像
            depth_image = capture.depth # 取得深度影像
                
            if color_image is None:
                print("沒有彩色影像")
                break

            if color_image.shape[-1] == 4:  # 如果影像是 RGBA 格式，移除 Alpha 通道
                color_image = color_image[..., :3]  # 保留前三個通道 (RGB)

            color_image = color_image.astype(np.uint8) # 確保數據類型為 uint8
            # BGR: color_image = color_image[..., ::-1]  # BGR = RGB[::-1]
            capture_array = np.array(color_image, dtype=np.uint8)  
            depth_array = np.array(depth_image, dtype=np.uint16) # 轉成 numpy array
                
            # 獲取校準資料
            calibration = self.k4a.calibration
            # 轉換
            transformed_depth_image = transformation.depth_image_to_color_camera(depth_array, calibration, False) # 取得跟彩色圖對齊的深度圖(720, 1280)
            '''   
            print("深度影像尺寸:", depth_image.shape)  # (height, width)   深度影像尺寸: (576, 640)
            print("彩色影像尺寸:", color_image.shape)  # (height, width, channel)   彩色影像尺寸: (720, 1280, 3)'''
            #----------------------
            if type(capture_array) == np.ndarray:
                #yolo==============================================================
                result = yolo(capture_array)
                if result is not None:
                    drawBox(result, capture_array, transformed_depth_image,
                            self.xh, self.yh, self.zh)
                # 輸出深度矩陣
                with open('depth.txt', 'w') as f2:
                    np.savetxt(f2, transformed_depth_image, fmt='%d')      
            else: #非可辨識圖片
                print('unknown img type')

            end=time.time()
            print(f"計算時間: {(end-start)*1000} 毫秒")

            # 更新共用畫面變數
            self.latest_frame = capture_array
            self.latest_depth = transformed_depth_image
        cv2.destroyAllWindows()                 # 結束所有視窗

    def update_frame(self):
        #ret, frame = self.cap.read()        if hasattr(self, "latest_frame"):
            capture_a = self.latest_frame
            # 如果有滑鼠位置，就畫深度數字
            if self.mouse_pos and hasattr(self, "latest_depth"):
                time.sleep(0.3)
                x, y = self.mouse_pos
                depth = self.latest_depth[y][x]
                
                # 偏移量，避免文字剛好疊在滑鼠上
                offset_x, offset_y = 10, -10
                text_pos = (x + offset_x, y + offset_y)
                

                cv2.putText(capture_a, f"{depth}", text_pos, cv2.FONT_HERSHEY_SIMPLEX, 1, (200, 100, 0), 2, cv2.LINE_AA)
                cv2.circle(capture_a, (x, y), 3, (0, 255, 0), -1)
                print(f"滑鼠深度: {depth}")

            # OpenCV 是 BGR，轉換為 RGB 給 QImage 用
            frame = cv2.cvtColor(capture_a, cv2.COLOR_BGR2RGB)
            h, w, ch = frame.shape
            bytes_per_line = ch * w
            qt_image = QImage(frame.data, w, h, bytes_per_line, QImage.Format_RGB888)
            self.label.setPixmap(QPixmap.fromImage(qt_image)) # 轉換成 QPixmap，才可以在 GUI 元件上顯示
   
    def closeEvent(self, event): # PyQt 在關閉窗口時會自動調用，不用註冊
        self.k4a.stop()  # 安全釋放 Kinect 資源
        event.accept()

if __name__ == '__main__':
    app = QApplication(sys.argv) # 建立QApplication物件，管理應用程式的初始化和事件循環
    window = CameraWindow()
    window.show()
    sys.exit(app.exec_()) # app.exec_()啟動PyQt的事件循環 回傳一個整數，代表應用程式的退出狀態